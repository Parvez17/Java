...
public class TeacherOfferedCourse extends JPanel{
	
	TeacherFrame frame;
	static Object[][] databaseInfo;
	static DefaultTableModel dTableModel;
	int ida;
	public TeacherOfferedCourse(TeacherFrame frame,int ida){
		this.frame=frame;
		this.ida=ida;
		this.setLayout(null);
    ...
    public void actionPerformed(ActionEvent e){  
			...
			frame.setVisible(false);
				...
				ArrayList<Courset> list = courseList();
	...
	public ArrayList<Courset> courseList(){
		
		ArrayList<Courset> courseList = new ArrayList<Courset>();
		...
		try{
			...
			while(rs.next())
			{
				course = new Courset(rs.getString(1), rs.getInt(2), rs.getInt(5),rs.getInt(3),rs.getInt(5),rs.getString(6));
