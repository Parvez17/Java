...
public class StudentChangePass extends JPanel implements ActionListener{
	...
	String spass;
	int ida;
	public StudentChangePass(StudentFrame frame,int ida){
		this.frame=frame;
		this.ida=ida;
		this.setLayout(null);
	...
	public void actionPerformed(ActionEvent e)
		...
		String chck="SELECT `password` FROM `student` WHERE `password`='"+s1+"' and `id`='"+ida+"'";	
		...
		if(spass.equals(s1))
		{
		int a=stmt.executeUpdate("UPDATE `student` SET `password`='"+s2+"' WHERE `id`='"+ida+"'");
		if(a==1)
		{
